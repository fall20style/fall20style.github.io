...
``` bash
mkdir -p ~/.gemini/skills/now-time
```
...
# Now Time Skill
현재 시간을 출력하기
- YYYY-MM-DD-HH:MM 포맷으로 적어주기
- 예제 시간 : 2026-05-05-16:50
```

### 3. 스킬 등록

- gemini-cli 설정 파일(~/.gemini/config.yaml 등)이나 시스템 경로에 해당 스킬 위치를 등록함.
- (현재 환경에서는 이미 /home/mjpark/.gemini/skills/now-time/SKILL.md에 위치해 있음.)
